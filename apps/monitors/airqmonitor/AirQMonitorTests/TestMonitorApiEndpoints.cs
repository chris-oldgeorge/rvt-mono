...
public class TestMonitorApiEndpoints
{
    ...
    {
        Mock<IAirQDateImporter> importer = new Mock<IAirQDateImporter>(MockBehavior.Strict);
        ...
        foreach (string? suppliedKey in new string?[] { null, "wrong-api-key" })
        {
            ...
            {
                using HttpRequestMessage request = new HttpRequestMessage(HttpMethod.Post, "/store-noise-levels-for-date")
                {
