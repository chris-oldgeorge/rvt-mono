    ...
    ILogger<ReportGenerationService> logger) : IReportGenerationService
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        // is the only thing that can say so. This snapshot is a fast path that
        // spares a lock round trip per already-done period; it is not the check
        // that decides, because it goes stale the moment a competing run
        // commits a period this one has not reached yet.
        ...
        foreach (ReportPeriod period in periods)
        {
            ...
            // Authoritative, and only meaningful here: holding the lock is what
            // makes the answer stable for as long as this period is generated.
            // Backfilling gives a competing run whole seconds - a PDF render, a
            // blob write, a narrative, an SMTP round per recipient - to finish
            // this period and release the lock while this run is busy with an
            // earlier one, and the recipients pay for a stale answer in
            // duplicate report emails.
            if (await _ruleQueries.HasGeneratedPeriodAsync(
                    rule.Id,
                    period.Frequency,
                    period.StartUtc,
                    cancellationToken).ConfigureAwait(false))
            {
                if (_logger.IsEnabled(LogLevel.Information))
                {
                    _logger.LogInformation(
                        "Report period {PeriodStartUtc} ({Frequency}) for rule {ReportRuleId} was generated by a competing run; skipping.",
                        period.StartUtc,
                        period.Frequency,
                        rule.Id);
                }
