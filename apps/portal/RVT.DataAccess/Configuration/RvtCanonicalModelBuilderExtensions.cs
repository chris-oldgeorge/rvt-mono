...
public static class RvtCanonicalModelBuilderExtensions
{
    ...
    private static void ApplyCanonicalColumnNames(ModelBuilder modelBuilder, IMutableEntityType entityType)
    {
        ...
        {
            bool isSingleColumnPrimaryKey = primaryKey is not null &&
                primaryKey.Properties.Count == 1 &&
                primaryKey.Properties.Contains(property);
            string sourceColumnName = property.GetColumnName() ?? property.Name;
            string columnName = isSingleColumnPrimaryKey
    ...
    {
        IMutableForeignKey? foreignKey = foreignKeys.FirstOrDefault(candidate =>
        ...
        string principalRelation = DatabaseNamingRules.ToCanonicalRelationName(
            foreignKey.PrincipalEntityType.GetTableName() ?? foreignKey.PrincipalEntityType.ClrType.Name);
        string principalField = DatabaseNamingRules.ToCanonicalColumnName(foreignKey.PrincipalKey.Properties[0].Name, true);
