...
public class NotificationAlertWorkflowTests
{
    ...
    public async Task NotificationLists_AreFilteredAndScoped()
    {
        ...
        QueryNotificationsResponse? open = await adminClient.GetFromJsonAsync<QueryNotificationsResponse>("/api/notifications?state=open");
        QueryNotificationsResponse? cautions = await adminClient.GetFromJsonAsync<QueryNotificationsResponse>("/api/notifications?state=cautions");
        QueryNotificationsResponse? siteScoped = await adminClient.GetFromJsonAsync<QueryNotificationsResponse>($"/api/notifications?siteId={ids.SiteId}");

        HttpClient companyClient = CreateClient(factory);
        await LoginAsync(companyClient, CompanyUserEmail, Password);
        QueryNotificationsResponse? companyOpen = await companyClient.GetFromJsonAsync<QueryNotificationsResponse>("/api/notifications?state=open");
        HttpResponseMessage hiddenOther = await companyClient.GetAsync($"/api/notifications/{ids.OtherAlertNotificationId}");
    ...
    public async Task NotificationDetailCloseAndBatchClose_UpdateVisibleAlertsOnly()
    {
        ...
        EntityResponse<NotificationDetailResponse>? detail = await client.GetFromJsonAsync<EntityResponse<NotificationDetailResponse>>($"/api/notifications/{ids.AlertNotificationId}");
        HttpResponseMessage close = await client.PostAsJsonAsync($"/api/notifications/{ids.AlertNotificationId}/close", new NotificationCloseRequest
        {
            Note = closeNote
        });
        EntityResponse<NotificationDetailResponse>? closed = await close.Content.ReadFromJsonAsync<EntityResponse<NotificationDetailResponse>>();
        HttpResponseMessage batch = await client.PostAsJsonAsync("/api/notifications/batch-close", new NotificationBatchCloseRequest
        {
            NotificationIds = [ids.OtherAlertNotificationId, ids.CautionNotificationId, Guid.NewGuid()],
            Note = "Batch close"
        });
        NotificationBatchCloseResponse? batchResult = await batch.Content.ReadFromJsonAsync<NotificationBatchCloseResponse>();
    ...
    public async Task CompanyUserCloseAlert_RecordsNoteAndRejectsOtherSiteAlerts()
    {
        ...
        EntityResponse<NotificationDetailResponse>? closed = await close.Content.ReadFromJsonAsync<EntityResponse<NotificationDetailResponse>>();
        HttpResponseMessage otherSiteBatch = await client.PostAsJsonAsync("/api/notifications/batch-close", new NotificationBatchCloseRequest
        {
            NotificationIds = [ids.OtherAlertNotificationId],
            Note = "Not my site"
        });
        NotificationBatchCloseResponse? batchResult = await otherSiteBatch.Content.ReadFromJsonAsync<NotificationBatchCloseResponse>();
    ...
    public async Task CompanyUserAlertLevels_RequireActiveAssignmentWindow()
    {
        ...
        HttpResponseMessage futureResponse = await futureClient.GetAsync($"/api/alert-levels?monitorId={ids.MonitorId}");

        HttpClient expiredClient = CreateClient(fixedTimeFactory);
        await LoginAsync(expiredClient, "alerts.expired@rvt.test", Password);
        HttpResponseMessage expiredResponse = await expiredClient.GetAsync($"/api/alert-levels?monitorId={ids.MonitorId}");

        HttpClient boundaryClient = CreateClient(fixedTimeFactory);
        await LoginAsync(boundaryClient, "alerts.boundary@rvt.test", Password);
        HttpResponseMessage boundaryResponse = await boundaryClient.GetAsync($"/api/alert-levels?monitorId={ids.MonitorId}");
    ...
    public async Task CompanyUserNotificationClose_RequiresActiveAssignmentWindow()
    {
        ...
        Guid boundaryNotificationId = Guid.NewGuid();
        ApplicationUser futureUser = await SeedCompanyUserAsync(factory, "close.future@rvt.test", ids.CompanyId);
        ...
        HttpResponseMessage futureClose = await CloseAsync(futureClient, futureNotificationId, "future assignment");

        HttpClient expiredClient = CreateClient(fixedTimeFactory);
        await LoginAsync(expiredClient, "close.expired@rvt.test", Password);
        HttpResponseMessage expiredClose = await CloseAsync(expiredClient, expiredNotificationId, "expired assignment");

        HttpClient boundaryClient = CreateClient(fixedTimeFactory);
        await LoginAsync(boundaryClient, "close.boundary@rvt.test", Password);
        HttpResponseMessage boundaryClose = await CloseAsync(boundaryClient, boundaryNotificationId, "boundary assignment");
    ...
    public async Task MovedMonitorNotifications_DoNotFallBackToCurrentDeployment()
    {
        ...
        HttpClient adminClient = CreateClient(factory);
        await LoginAsync(adminClient, AdminEmail, Password);
        EntityResponse<NotificationDetailResponse>? adminDetail = await adminClient.GetFromJsonAsync<EntityResponse<NotificationDetailResponse>>($"/api/notifications/{ids.GapNotificationId}");

        HttpClient companyClient = CreateClient(factory);
        await LoginAsync(companyClient, CompanyUserEmail, Password);
        QueryNotificationsResponse? companyOpen = await companyClient.GetFromJsonAsync<QueryNotificationsResponse>("/api/notifications?state=open");
        HttpResponseMessage companyDetail = await companyClient.GetAsync($"/api/notifications/{ids.GapNotificationId}");
        HttpResponseMessage companyClose = await companyClient.PostAsJsonAsync($"/api/notifications/{ids.GapNotificationId}/close", new NotificationCloseRequest
        {
    ...
    public async Task AlertLevelCrud_ValidatesCreatesUpdatesAndDeletesDustNoiseLevels()
    {
        ...
        HttpResponseMessage invalid = await client.PostAsJsonAsync("/api/alert-levels", new AlertLevelMutationRequest
        {
        ...
        HttpResponseMessage create = await client.PostAsJsonAsync("/api/alert-levels", new AlertLevelMutationRequest
        {
            MonitorId = ids.MonitorId,
            AlertField = "pm10",
            AlertType = AlertTypeEnum.Alert.ToString(),
        ...
        EntityResponse<AlertLevelItem>? created = await create.Content.ReadFromJsonAsync<EntityResponse<AlertLevelItem>>();
        HttpResponseMessage update = await client.PutAsJsonAsync($"/api/alert-levels/{created!.Item!.Id}", new AlertLevelMutationRequest
        {
            MonitorId = ids.MonitorId,
            AlertField = "pm2.5",
        ...
        EntityResponse<AlertLevelItem>? updated = await update.Content.ReadFromJsonAsync<EntityResponse<AlertLevelItem>>();
        HttpResponseMessage delete = await client.DeleteAsync($"/api/alert-levels/{created.Item.Id}");
        QueryAlertLevelsResponse? list = await client.GetFromJsonAsync<QueryAlertLevelsResponse>($"/api/alert-levels?monitorId={ids.MonitorId}");
    ...
    public async Task VibrationAlertLevelEndpoint_UpsertsAlertAndCautionPair()
    {
        ...
        HttpResponseMessage invalid = await client.PutAsJsonAsync($"/api/alert-levels/monitors/{ids.VibrationMonitorId}/vibration", new VibrationAlertLevelMutationRequest
        {
            AlertLevel = 5,
            CautionLevel = 6
        });
        ...
        HttpResponseMessage create = await client.PutAsJsonAsync($"/api/alert-levels/monitors/{ids.VibrationMonitorId}/vibration", new VibrationAlertLevelMutationRequest
        {
            AlertLevel = createAlertLevel,
            CautionLevel = 8
        });
        VibrationAlertLevelResponse? created = await create.Content.ReadFromJsonAsync<VibrationAlertLevelResponse>();
        HttpResponseMessage update = await client.PutAsJsonAsync($"/api/alert-levels/monitors/{ids.VibrationMonitorId}/vibration", new VibrationAlertLevelMutationRequest
        {
            AlertLevel = updatedAlertLevel,
            CautionLevel = updatedCautionLevel
        });
        VibrationAlertLevelResponse? updated = await update.Content.ReadFromJsonAsync<VibrationAlertLevelResponse>();
        QueryAlertLevelsResponse? list = await client.GetFromJsonAsync<QueryAlertLevelsResponse>($"/api/alert-levels?monitorId={ids.VibrationMonitorId}");
        ...
        DateTime? scenarioNowUtc = null)
    {
        ...
        Guid noiseMonitorId = Guid.NewGuid();
        Guid vibrationMonitorId = Guid.NewGuid();
        Guid otherMonitorId = Guid.NewGuid();
        Guid alertNotificationId = Guid.NewGuid();
        Guid cautionNotificationId = Guid.NewGuid();
        Guid otherAlertNotificationId = Guid.NewGuid();
        DateTime now = scenarioNowUtc ?? DateTime.UtcNow;
