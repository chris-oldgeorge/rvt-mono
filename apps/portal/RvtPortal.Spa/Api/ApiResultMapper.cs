// File summary: Translates application-boundary use-case results into ASP.NET Core API responses.
// Major updates:
// - 2026-07-30 pending Collapsed the ApplicationResult overloads after RVT.BusinessLogic dissolved into RvtPortal.Application.
...
{
    // Function summary: Converts an application-boundary result into a typed controller action result.
    ActionResult<TResponse> ToActionResult<TModel, TResponse>(
        ControllerBase controller,
        UseCaseResult<TModel> result,
...
public sealed class ApiResultMapper : IApiResultMapper
{
        ...
        Func<TModel, TResponse> map)
    {
        ...
        {
            UseCaseResultKind.Success when result.Value is not null => map(result.Value),
            UseCaseResultKind.NotFound => controller.NotFound(CreateProblem(
                controller,
                StatusCodes.Status404NotFound,
                "Resource not found.",
                result.Message)),
            UseCaseResultKind.Forbidden => controller.Forbid(),
            UseCaseResultKind.Validation => controller.BadRequest(new ValidationProblemDetails(
                result.Errors
                    .GroupBy(error => error.Field)
                    .ToDictionary(group => group.Key, group => group.Select(error => error.Message).ToArray()))),
            UseCaseResultKind.Conflict => controller.Conflict(CreateProblem(
                controller,
                StatusCodes.Status409Conflict,
                "Request conflict.",
                result.Message)),
            UseCaseResultKind.ExternalServiceUnavailable => ExternalServiceProblem(controller, result),
