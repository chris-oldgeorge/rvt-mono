...
public sealed class ReportRuleApplicationService : IReportRuleApplicationService
{
    ...
    public async Task<UseCaseResult<ReportUserAssignmentModel>> AddUserAsync(Guid id, Guid userId, CancellationToken cancellationToken)
    {
        ...
        if (!exists)
        {
            _searchContext.ReportUsers.Add(new ReportUser
            {
                ReportRuleId = id,
                UserId = userId
            });
    ...
    public async Task<UseCaseResult<ReportUserAssignmentModel>> RemoveUserAsync(Guid id, Guid userId, CancellationToken cancellationToken)
    {
        ...
        {
            _searchContext.ReportUsers.RemoveRange(assignments);
            await _searchContext.SaveChangesAsync(cancellationToken);
        }
    ...
    public async Task<UseCaseResult<ReportGenerationResponseModel>> RequestGenerationAsync(
        ...
        CancellationToken cancellationToken)
    {
        bool exists = await _searchContext.ReportRules
            .AsNoTracking()
    ...
    private async Task<ReportRuleDetailModel> BuildRuleDetailAsync(ReportRule rule, CancellationToken cancellationToken)
    {
        ...
        return new ReportRuleDetailModel
        {
            ...
            AssignedUserCount = await _searchContext.ReportUsers.CountAsync(user => user.ReportRuleId == rule.Id, cancellationToken)
        };
    }
    ...
    {
        List<ReportRuleOptionModel> sites = await _domainContext.Sites
            .AsNoTracking()
    ...
    {
        return await _domainContext.HelpArticles
            .AsNoTracking()
    ...
    private async Task<Dictionary<Guid, string>> BuildSiteNamesAsync(IEnumerable<Guid> siteIds, CancellationToken cancellationToken)
    {
        ...
        return await _domainContext.Sites
            .AsNoTracking()
