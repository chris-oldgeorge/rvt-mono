...
public sealed class ReportRuleRecipientReader : IReportRuleRecipientReader
{
    ...
    public async Task<QueryReportUsersResponse?> QueryAssignmentUsersAsync(
        ...
        CancellationToken cancellationToken)
    {
        ReportAssignmentContext? assignmentContext = await LoadAssignmentContextAsync(reportRuleId, cancellationToken);
        if (assignmentContext == null)
        {
            return null;
        }

        IReadOnlyList<string> assignedUserIds = await LoadAssignedUserIdsAsync(reportRuleId, cancellationToken);
        IReadOnlyList<string> visibleUserIds = await LoadVisibleUserIdsAsync(assignmentContext.SiteId, assignedUserIds, cancellationToken);
        string requestedSort = string.IsNullOrWhiteSpace(request.Sort) ? "email" : request.Sort.Trim();
        int assignedUserCount = await BuildCandidateUserQuery(visibleUserIds, assignedUserIds, true).CountAsync(cancellationToken);
        IQueryable<UserSearch> users = BuildCandidateUserQuery(visibleUserIds, assignedUserIds, assigned);
        IQueryable<UserSearch> query = ApplyUserSearch(users, request.SearchText);
        ...
        List<UserSearch> rows = await query.Skip((page - 1) * pageSize).Take(pageSize).ToListAsync(cancellationToken);
        List<UserListItem> results = await BuildUserItemsFromRowsAsync(rows, cancellationToken);
        ...
        IReadOnlyDictionary<Guid, int> siteCounts)
    {
        ...
        return new UserListItem
        {
            ...
            Role = user.Role ?? "",
            SiteCount = parsedId == Guid.Empty || !siteCounts.TryGetValue(parsedId, out int siteCount) ? 0 : siteCount,
        ...
        CancellationToken cancellationToken)
    {
        ...
        List<string> adminUserIds = await _applicationContext.UserRoles
            .AsNoTracking()
            .Join(
                _applicationContext.Roles.AsNoTracking().Where(role => _adminRoleNames.Contains(role.Name)),
                userRole => userRole.RoleId,
                role => role.Id,
        ...
        HashSet<string> candidateIds = visibleUserIdStrings
            .Concat(adminUserIds)
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (ApplicationUser user in users)
        {
            ...
            items.Add(new UserListItem
            {
                Id = user.Id,
                CompanyId = user.CompanyId,
                CompanyName = user.CompanyId.HasValue && companies.TryGetValue(user.CompanyId.Value, out string? companyName) ? companyName : null,
                IsDisabled = user.IsDisabled,
                Name = user.Name,
                Email = user.Email ?? "",
                PhoneNumber = user.PhoneNumber,
