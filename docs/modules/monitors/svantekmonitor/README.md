...
Uses monitor applications with timer triggers to collext Svantek web device and noise data
...
StoreNoiseLevels - [timerTrigger] Retrieves and Stores sample data for stored devices
    Svantek updates the data every 15 minutes on the quarter hour
...
StoreNoiseLevelsForDate - [httpTrigger]
    Invoke url: https://svantekmonitor.azurewebsites.net/api/storenoiselevelsfordate

e.g.  Http GET
https://svantekmonitor.azurewebsites.net/api/StoreNoiseLevelsForDate?user_id=<userid>d&user_auth=<token>&date=<YYYY-MM-DD>"

Would store all samples from listed devices on the given date

scripts in SvantekMonitorTests have been used to test this.  
  

## Project Setup

Monitor application for collection of sensor data from Svantek/Turnkey API

API Docs https://datacollector.svantekweb.com/swagger-ui/

## Shared communications

Svantek resolves email and SMS through the common provider-neutral adapters. Configure `RVT__EMAIL_ENABLED`, `RVT__EMAIL_PROVIDER`, the selected SendGrid or Microsoft Graph settings, and `RVT__SMS_ENABLED` plus TransmitSMS settings as documented in the root README. Docker Compose disables both channels by default so local startup never requires a credential. Durable delivery is at least once; transient failures retry and permanent/configuration failures dead-letter without persisting provider secrets or destinations.
...
In directory SvantekMonitorTests in terminal run:
...
'''
func monitor applicationapp publish SvantekMonitor
'''
