...
| Help Admin | Deliberately excluded from the release surface; no deployment workflow depends on its temporary asset editor. | `RvtPortal.Client/src/admin/HelpAdminPanel.tsx` remains present for internal development only. | No cutover sign-off is required while excluded. Re-evaluate and add the stable row-key regression before any future shipment. | EXCLUDED |
