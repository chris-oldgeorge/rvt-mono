...
| # | Finding | Evidence |
|---|---------|----------|
| P9 | **RESOLVED by PR #23** (verified): the bare catch now logs genuine failures and propagates cancellation. `catch { return null; }` with no OCE filter and no logging — cancellation and real failures both degrade silently to "no metric". (The sibling `SiteArchiveAdapter` got the OCE filter; this didn't.) | `MonitorDetailSummaryService.cs:165-168` |
| P10 | **RESOLVED by PR #23** (verified): a logger is now injected and failures are logged. `SiteArchiveAdapter` maps any failure to a fixed message with no logger injected — improved (OCE rethrown) but still undiagnosable. | `SiteArchiveAdapter.cs:27-30,50-53` |
...
| P16 | **RESOLVED by PR #23** (verified): the personal recipient is gone and `AllowedSerialIds` ships empty. Personal email `haakan.eriksson@cellsoftware.co.uk` and `AllowedSerialIds: ["23423"]` + `MaxMonitorsPerRun: 1` still committed as Omnidots defaults. | `OmnidotsMonitor/appsettings.json:13,21-24` |
...
| P21 | **RESOLVED on main** (verified: the solution now builds with 0 warnings). Obsolete-plumbing hygiene: the shared libs themselves lack the documented `RVT0001` NoWarn (7 undocumented warnings per full build — trains people to ignore the retirement diagnostic); the sync *members'* `[Obsolete]` lacks a `DiagnosticId`, so tests suppress `CS0618` while the interface uses `RVT0001` — two warning IDs for one legacy path. | `Rvt.Communication`, `RuleAlertNotificationDispatcher.cs:11,16` |
