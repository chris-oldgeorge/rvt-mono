...
public sealed class MicrosoftGraphEmailAdapter : IEmailDeliveryPort
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (httpClientFactory is not null)
        {
            ...
            MicrosoftGraphEmailAdapter operationAdapter = new MicrosoftGraphEmailAdapter(
                operationClient,
                tokenProvider,
                options);
        ...
        GraphSendMailRequest payload = new GraphSendMailRequest(
            new GraphMessage(
                request.Subject,
                new GraphItemBody(
        ...
        CancellationToken cancellationToken)
    {
        ...
        GraphMessage draft = new GraphMessage(
            request.Subject,
            body,
            [new GraphRecipient(new GraphEmailAddress(request.Recipient))],
            null);
        string senderPath = $"users/{Uri.EscapeDataString(options.SenderAddress)}";
        Uri draftUri = new Uri(GraphBaseUri, $"{senderPath}/messages");
    ...
    private async Task<MicrosoftGraphUploadSession> CreateUploadSessionAsync(
        ...
        CancellationToken cancellationToken)
    {
        GraphUploadSessionRequest request = new GraphUploadSessionRequest(new GraphAttachmentItem(
            "file",
            attachment.FileName,
            attachment.Length,
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (offset < attachment.Length)
        {
            ...
            using HttpRequestMessage message = new HttpRequestMessage(HttpMethod.Put, session.UploadUrl);
