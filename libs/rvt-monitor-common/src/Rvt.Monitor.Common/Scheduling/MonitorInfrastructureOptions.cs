...
public sealed record MonitorInfrastructureOptions
{
    ...
    {
        string? configuredValue = configuration["Infrastructure"]
        ...
        if (!Enum.TryParse<MonitorInfrastructure>(configuredValue, ignoreCase: true, out MonitorInfrastructure infrastructure))
        {
